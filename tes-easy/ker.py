...
def kerasocr_to_lines(predictions):
    """
    Конвертирует результат keras OCR в список строк.
    
    Args:
        predictions: список предсказаний keras OCR (word, box)
    
    Returns:
        Список распознанных строк текста
    """
    # Если нет предсказаний, возвращаем пустой список
    if not predictions:
        return []
    
    # Сортируем предсказания по Y-координате (сверху вниз)
    # Берем среднюю Y-координату bounding box
    predictions_with_y = []
    for word, box in predictions:
        y_coords = [point[1] for point in box]
        avg_y = np.mean(y_coords)
        predictions_with_y.append((word, avg_y, box))
    
    # Сортируем по Y
    predictions_with_y.sort(key=lambda x: x[1])
    
    # Группируем по строкам (предполагая, что строки разделены по Y-координате)
    lines = []
    current_line = []
    current_y = None
    y_threshold = 20  # Порог для определения новой строки
    
    for word, y, box in predictions_with_y:
...
def process_image(image_path, pipeline, save_intermediate=False):
    print(f"\n{'='*50}")
    ...
    # Конвертируем BGR в RGB для keras-ocr
    ...
    if save_intermediate:
        ...
        cv2.imwrite(f"debug/{base_name}_marks_kocr.png", draw)
    
    if roi is None or roi.size == 0:
        print("Ошибка: ROI пустой, используем оригинальное изображение")
        roi = img.copy()
    
    # Конвертируем ROI в RGB
    roi_rgb = cv2.cvtColor(roi, cv2.COLOR_BGR2RGB)
    
    if save_intermediate:
        cv2.imwrite(f"debug/{base_name}_roi_kocr.png", roi)
    
    # OCR с использованием keras-ocr
    # keras-ocr работает напрямую с изображениями в RGB формате
    predictions = pipeline.recognize([roi_rgb])[0]
    
    # Конвертируем предсказания в строки
    lines = kerasocr_to_lines(predictions)
    
    # Фильтруем пустые строки
    lines = [l.strip() for l in lines if l.strip()]
    
    # Сохраняем распознанный текст
    text_file = os.path.splitext(image_path)[0] + "_kocr" + ".txt"
    with open(text_file, "w", encoding="utf-8") as file:
        file.write('\n'.join(lines))
    
    # Сохраняем визуализацию OCR (опционально)
    if save_intermediate:
        fig, ax = plt.subplots(figsize=(10, 10))
        keras_ocr.tools.drawAnnotations(image=roi_rgb, predictions=predictions, ax=ax)
        plt.savefig(f"debug/{base_name}_ocr_kocr.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
    ...
    if min_lines == 0:
        ...
        print(f"Ожидалось строк: {len(expected_lines)}, найдено: {len(lines)}")
        print(f"Найденные строки: {lines}")
        return None
    ...
    print(f"Найдено OCR строк: {file_metrics['ocr_lines_found']}")
    
    return file_metrics

def main():
    # Инициализация pipeline keras-ocr
    print("Инициализация keras-ocr pipeline...")
    pipeline = keras_ocr.pipeline.Pipeline()
    print("keras-ocr pipeline инициализирован")
    ...
    for image_file in filtered_files:
        metrics = process_image(image_file, pipeline, save_intermediate=True)
        if metrics:
    ...
    df_metrics.to_csv("results/file_metrics_kocr.csv", index=False, encoding='utf-8-sig')
    df_lines.to_csv("results/line_metrics_kocr.csv", index=False, encoding='utf-8-sig')
    
    with open("results/summary_report_kocr.txt", "w", encoding="utf-8") as f:
        f.write("ОТЧЕТ ПО ОБРАБОТКЕ OCR (keras-ocr)\n")
        f.write("="*50 + "\n\n")
