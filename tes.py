...
def process_image(image_path, save_intermediate=False):
    ...
    if save_intermediate:
        ...
        cv2.imwrite(f"debug/{base_name}_marks_tes.png", draw)
    ...
    if save_intermediate:
        cv2.imwrite(f"debug/{base_name}_binary_tes.png", thresh)
        cv2.imwrite(f"debug/{base_name}_roi_tes.png", roi)
    ...
    text_file = os.path.splitext(image_path)[0] + "_tes"+ ".txt"
...
def main():
    ...
    df_metrics.to_csv("results/file_metrics_tes.csv", index=False, encoding='utf-8-sig')
    df_lines.to_csv("results/line_metrics_tes.csv", index=False, encoding='utf-8-sig')
    
    with open("results/summary_report_tes.txt", "w", encoding="utf-8") as f:
        f.write("ОТЧЕТ ПО ОБРАБОТКЕ OCR\n")
        f.write("="*50 + "\n\n")
